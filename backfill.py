...
async def run_backfill(client: Client, audit_file: str = None):
    ...
    print(
        f"Listing blobs in gs://{bucket_name}/prod/knowledge/v1/ to find completed files..."
    )
    prod_blobs = gcs_client.list_blobs(bucket_name, prefix="prod/knowledge/v1/")
    existing_prod_files = set(blob.name.split("/")[-1] for blob in prod_blobs if blob.name.endswith(".parquet"))
    print(
        f"Found {len(existing_prod_files)} already completed files in prod/knowledge/v1/."
    )
    ...
    if audit_file and os.path.exists(audit_file):
        print(f"Loading missing documents from {audit_file}...")
        with open(audit_file, "r") as f:
            missing_docs = json.load(f)
        for doc_dict in missing_docs:
            documents.append(KnowledgeDocument(**doc_dict))
    else:
    ...
    for blob in blobs:
        ...
        if len(parts) >= 5:
            ...
            expected_prod_filename = f"{doc_id}.parquet"

            if expected_stage_path in existing_stage_files:
                continue  # Skip already processed files
            if expected_prod_filename in existing_prod_files:
                continue  # Skip files already in prod
...
async def main():
    parser = argparse.ArgumentParser(description="Backfill missing knowledge documents.")
    parser.add_argument("--audit-file", type=str, help="Path to missing_documents.json generated by audit_integrity.py")
    ...
    try:
        await run_backfill(client, args.audit_file)
