...
def get_latest_companies() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        blobs = list(
            client.list_blobs(COMPANIES_GCS_BUCKET, prefix=COMPANIES_GCS_PREFIX)
        )
        ...
        if not company_blobs:
            raise FileNotFoundError(
                f"No companies.json found under gs://{COMPANIES_GCS_BUCKET}/{COMPANIES_GCS_PREFIX}"
            )
        ...
        logger.info(
            f"Loading companies from: gs://{COMPANIES_GCS_BUCKET}/{latest_blob.name}"
        )
